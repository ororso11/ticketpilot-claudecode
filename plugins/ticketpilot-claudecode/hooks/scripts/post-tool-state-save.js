...
// Records tool use events to trace.jsonl when a TicketPilot workflow is active.
...
try {
  ...
  // Only act if there's an active workflow
  ...
  // Track file writes to changedFiles
  if (['Write', 'Edit', 'write_file', 'edit_file'].includes(toolName)) {
    ...
    if (filePath && state.changedFiles) {
      if (!state.changedFiles.includes(filePath)) {
        ...
        appendTrace('state_auto_saved', `changedFiles updated: +${filePath}`, state.ticketKey, state.phase);
      }
    }
  }
  ...
  console.error('[TicketPilot] post-tool-state-save hook error (non-fatal):', err?.message ?? err);
  process.exit(0);
}
