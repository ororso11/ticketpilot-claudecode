...
export class JiraClient {
    ...
    async request(path) {
        ...
        if (!response.ok) {
            ...
            if (status === 403)
                throw new Error('Jira access denied. Your account may lack permission for this resource.');
            if (status === 404)
                throw new Error(`Jira resource not found at: ${path}`);
            if (status === 429)
                throw new Error('Jira rate limit exceeded. Please wait and try again.');
    ...
    async getIssue(issueKey) {
        const raw = await this.request(`/rest/api/3/issue/${issueKey}`);
    ...
    async getComments(issueKey) {
        const raw = await this.request(`/rest/api/3/issue/${issueKey}/comment`);
    ...
    async testConnection() {
        try {
            const raw = await this.request('/rest/api/3/myself');
