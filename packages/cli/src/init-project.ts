...
async function detectPackageManager(root: string): Promise<string> {
  if (await fse.pathExists(path.join(root, 'pnpm-lock.yaml'))) return 'pnpm';
  if (await fse.pathExists(path.join(root, 'yarn.lock'))) return 'yarn';
  if (await fse.pathExists(path.join(root, 'package-lock.json'))) return 'npm';
  if (await fse.pathExists(path.join(root, 'bun.lockb'))) return 'bun';
  if (await fse.pathExists(path.join(root, 'Pipfile'))) return 'pipenv';
  if (await fse.pathExists(path.join(root, 'poetry.lock'))) return 'poetry';
  if (await fse.pathExists(path.join(root, 'pom.xml'))) return 'maven';
  if (await fse.pathExists(path.join(root, 'build.gradle'))) return 'gradle';
  return 'unknown';
}

async function detectTechStack(root: string): Promise<{ stack: string; frameworks: string[] }> {
...
async function detectBuildCommand(root: string, packageManager: string): Promise<string> {
  if (packageManager === 'pnpm' || packageManager === 'npm' || packageManager === 'yarn' || packageManager === 'bun') {
    try {
      const pkg = await fse.readJson(path.join(root, 'package.json'));
      if (pkg.scripts?.build) return `${packageManager} run build`;
    } catch { /* ignore */ }
  }
...
async function detectTestCommand(root: string, packageManager: string): Promise<string> {
  if (['pnpm', 'npm', 'yarn', 'bun'].includes(packageManager)) {
    ...
    if (await fse.pathExists(path.join(root, 'vitest.config.ts'))) return 'npx vitest run';
    if (await fse.pathExists(path.join(root, 'jest.config.ts')) || await fse.pathExists(path.join(root, 'jest.config.js'))) return 'npx jest';
  }
  if (packageManager === 'maven') return 'mvn test';
  if (packageManager === 'gradle') return './gradlew test';
  if (packageManager === 'pipenv' || packageManager === 'poetry') return 'pytest';
  return 'unknown';
}

async function detectImportantPaths(root: string): Promise<string[]> {
  const candidates = ['src', 'app', 'lib', 'packages', 'services', 'api', 'server', 'client', 'frontend', 'backend', 'core', 'domain'];
  const found: string[] = [];
  for (const c of candidates) {
    if (await fse.pathExists(path.join(root, c))) found.push(c);
  }
  return found;
}

async function detectRiskAreas(root: string): Promise<string[]> {
  const found: string[] = [];
  ...
  for (const entry of entries) {
    for (const r of riskDirs) {
      if (entry.toLowerCase().includes(r)) found.push(entry);
    }
  }
  return [...new Set(['auth', 'security', 'payment', 'privacy', 'db migration', 'production config', ...found])];
}

function buildProjectAnalysisDoc(memory: ProjectMemory, ticketpilotJiraProject: string): string {
...
function buildAgentsMd(memory: ProjectMemory): string {
...
export async function runInitProject(): Promise<void> {
